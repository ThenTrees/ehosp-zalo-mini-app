...
- `app-config.json` — app `title` (rendered in the Header via `getConfig(c => c.app.title)`), status bar, safe-area flags, and `template.oaID` (the Zalo OA opened by the chat CTA). `.vscode/settings.json` carries a JSON schema for it.
